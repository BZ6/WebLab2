import {handleShots} from "../shot/shotHandler.js";
...
const R = 150
const scale = canvas.width / (2.5 * R)
const ctx = canvas.getContext('2d')

function getShotR() {
    return sessionStorage.r ? parseFloat(JSON.parse(sessionStorage.r)) : 0 // Исправить попадание без радиуса
}
...
export function drawPlot() {
    ...
    // triangle
    ctx.beginPath()
    ctx.moveTo(transformX(R), transformY(0))
    ctx.lineTo(transformX(0), transformY(0))
    ctx.lineTo(transformX(0), transformY(-R / 2))
    ctx.closePath()
    ctx.fill()

    // semicircle
    ctx.beginPath()
    ctx.arc(transformX(0), transformY(0), (R / 2) * scale, Math.PI, 3 * Math.PI / 2, false)
    ctx.lineTo(transformX(0), transformY(0))
    ctx.closePath()
    ctx.fill()
